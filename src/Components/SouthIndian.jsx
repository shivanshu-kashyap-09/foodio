...
const SouthIndian = ({ southDish }) => {
  ...
  useEffect(() => {
    if (southDish && southDish.length > 0 && dishes.length === 0) {
      setDishes(southDish);
      setLoading(false);
      setHasMore(southDish.length >= 12);
    } else if (southDish && southDish.length === 0) {
      setLoading(false);
    }
  }, [southDish]);
  ...
  const handleSeeMore = async () => {
    ...
    console.log(`Fetching more South Indian dishes for ${currentPage + 1}...`);
    setLoadingMore(true);
    try {
      const nextPage = currentPage + 1;
      const response = await axios.get(`${import.meta.env.VITE_URL}/menus/southindian?page=${nextPage}&limit=12`);
      ...
      if (response.status === 200 && response.data.success) {
        ...
        if (!newItems || newItems.length === 0) {
          ...
          setDishes(prev => {
            ...
            console.log("Total South Indian dishes in state now:", updated.length);
            return updated;
          });
      ...
      console.error("Error loading more south indian dishes:", error);
    } finally {
  ...
  const displayItems = useMemo(() => {
    ...
    let filtered = dishes.map(dish => ({
      ...dish,
      restaurant_id: dish.res_id || dish.restaurant_id || 4
    }));
  ...
  return (
    <div className="py-12 sm:py-20 bg-orange-50/30 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-2">
            ...
            >
              ...
              <span className="text-orange-600 font-black text-[10px] uppercase tracking-[0.2em]">Tradition</span>
            </motion.div>
            <motion.h2 
            ...
            >
              FLAVORS OF <span className="text-orange-600 italic">SOUTH</span>
            </motion.h2>
            <p className="text-gray-500 font-medium max-w-md">Experience the real zest of the coastal south with our handpicked regional specialties.</p>
          </div>
            ...
            <div className="relative flex items-center" ref={searchRef}>
                ...
                {searchVisible && (
                  ...
                  >
                    <input
                      ...
                      placeholder="Search south indian..."
                      className="w-48 sm:w-64 px-4 py-3 bg-white border border-orange-100 rounded-2xl text-xs font-bold text-gray-700 shadow-sm focus:ring-2 focus:ring-orange-500 outline-none transition-all"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <button
                ...
                className={`w-12 h-12 flex items-center justify-center rounded-2xl transition-all shadow-sm search-button ${
                  searchVisible ? 'bg-orange-600 text-white' : 'bg-white text-gray-400 hover:text-orange-600 hover:bg-orange-50'
                }`}
              >
            ...
            <div className="relative" ref={filterRef}>
              <button
                ...
                className={`h-12 px-6 flex items-center gap-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-sm filter-button ${
                  filter ? 'bg-orange-600 text-white' : 'bg-white text-gray-600 hover:bg-orange-50'
                }`}
              >
                <FaFilter className={filter ? 'text-white' : 'text-gray-400'} />
                Filter
              </button>
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="py-20 text-center bg-white rounded-[3rem] border-2 border-dashed border-orange-100"
            >
              <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center mx-auto mb-6 text-orange-500 text-3xl">
                <FaSearch />
              </div>
              <h3 className="text-xl font-black text-gray-800 uppercase tracking-tighter">No soul food found</h3>
              <p className="text-gray-400 text-sm mt-2 font-medium">Try searching for dosa or idli favorites</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {!searchQuery && hasMore && (
          ...
          >
            <button
              ...
              className="group flex items-center gap-4 px-10 py-5 bg-white border-2 border-orange-100 rounded-full font-black text-xs uppercase tracking-widest text-gray-600 hover:border-orange-600 hover:text-orange-600 transition-all shadow-sm disabled:opacity-50"
            >
              {loadingMore ? 'Loading...' : 'Discover More Classics'}
