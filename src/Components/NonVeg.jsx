  ...
  return (
    <div className='bg-red-50'>
      <div className='relative flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 mb-6 px-4 sm:px-6 md:px-8'>
        <h2 className='text-red-900 font-bold text-2xl sm:text-3xl md:text-4xl text-left sm:ml-[40%]'>NONVEG FOOD</h2>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 mx-4 sm:mx-6 md:mx-8 lg:mx-12'>
        {(Array.isArray(searchVisible && searchQuery && filteredData.length > 0 ? filteredData : nonVegDish)
          ? (searchVisible && searchQuery && filteredData.length > 0 ? filteredData : nonVegDish)
